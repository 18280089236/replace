...
public class JsonpFileUtils {
    ...
    public static int replace(Map<String, String> properties, Elements allElements) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (element.ownText() == null || element.ownText().trim().equals("")) {
                continue;
            }
            //查找是否包含map键
            String textContent = element.ownText().trim().replace("\t","").replace("\n","");

            properties.forEach((k, v) -> {
                if (textContent.length() < 20 && textContent.equals(k)) {
